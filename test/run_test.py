...
def test_queries() -> Optional[List[Dict[str, Any]]]:
    queries = [
        # === 기본 검색 테스트 (필터 없음) ===
        {
            "query": "플레이어 이동 처리 함수",
        ...
        },
        {
            ...
            "description": "복합 필터 - C++ + function + project2"
        },
        {
            "query": "캐릭터 스탯 게임 데이터",
            "top_k": 10,
...
private:
    int value = 1;
        ...
        if not result or not any("version 1" in r.get("content", "") for r in result):
